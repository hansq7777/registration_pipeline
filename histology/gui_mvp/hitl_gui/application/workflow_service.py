...
class WorkflowService:
    ...
    def generate_mask_preview(self, idx: int, crop_level: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        slide = self.require_slide()
        proposal = slide.proposals[idx]
        crop_rgb = self.session_cache.get_section_crop(proposal, crop_level)
        tissue, artifact = compute_auto_masks(crop_rgb, slide.stain)
    ...
    def get_section_for_edit(self, idx: int, crop_level: int) -> tuple[ProposalBox, np.ndarray, np.ndarray, np.ndarray]:
        slide = self.require_slide()
        proposal = slide.proposals[idx]
        crop_rgb = self.session_cache.get_section_crop(proposal, crop_level)
